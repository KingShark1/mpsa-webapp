...
def create_lane_chart(chart_name='Lane_order_MPSA_2023'):
  ...
  day = int(input("Enter the day you want the Event Chart to be prepared for : ")) - 1
  
  row = 0
  worksheet = workbook.add_worksheet(days[day])
  for sn in range(len(data[days[day]])):
    cur_event = data[days[day]].iloc[sn]
    ...
    cur_df = pd.read_csv(f"data/csv_event_list/{days[day]}/{cur_event_name}.csv")
